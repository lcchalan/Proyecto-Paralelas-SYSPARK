                ...
                <div class="row">
                    <div class="col-md-6 col-sm-6 scrollpoint sp-effect1">
                        <form role="form" method="POST" action="return false" onsubmit="return false">
                            <?php 
                            $consulta="select * from administrador where CedulaAdmin=".$_SESSION["CedulaAdmin"];
                            $resultado=$conexion->query($consulta);
                            while ($fila=$resultado->fetch_assoc()) {    
                                ?>                                   
                                <div class="form-group">
                                    <p>Nombre su parqueadero</p>
                                    <input type="text" class="form-control" name="NombreAdminU" id="NombreAdminU" value='<?php echo $fila["NombreAdmin"] ?>' >
                                </div>
                                <div class="form-group">
                                    <p>Apellido</p>
                                    <input type="text" class="form-control" name="ApellidoAdminU" id="ApellidoAdminU" value='<?php echo $fila["ApellidoAdmin"] ?>'><br>

                                </div>
                                <div class="form-group">
                                    <p>Telefono Administrador</p>                                        
                                    <input type="text" class="form-control" name="TelefonoAdminU" id="TelefonoAdminU" value='<?php echo $fila["TelefonoAdmin"] ?>'>
                                </div>

                                <div class="form-group">
                                    <p>Correo Administrador</p>
                                    <input type="text" class="form-control" name="CorreoAdminU" id="CorreoAdminU" value='<?php echo $fila["CorreoAdmin"] ?>'><br>
                                </div>   
                                <div class="form-group">
                                    <p>Clave Administrador</p>
                                    <input type="password" class="form-control" name="ClaveAdminU" id="ClaveAdminU" value='<?php echo $fila["ClaveAdmin"] ?>'>
                                </div>                                                                
                                <?php } ?>

                                <button type='submit' onclick="updatePerfil( 
                                document.getElementById('NombreAdminU').value,
                                document.getElementById('ApellidoAdminU').value,                                                    
                                document.getElementById('TelefonoAdminU').value, 
                                document.getElementById('CorreoAdminU').value, 
                                document.getElementById('ClaveAdminU').value
                                );" class="btn btn-primary btn-lg" data-toggle='modal' data-target='.update'>Enviar</button>
                            </form>
                        </div>
                    </div>
                </div>
